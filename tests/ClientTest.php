...
class ClientTest extends TestCase
{
    ...
    public function testLanguages()
    {
        ...
        $this->assertEquals(['nl-nl', 'nl', 'en-us', 'en'], $client->languages());
    }
    ...
    public function testLanguagesSorted()
    {
        ...
        $this->assertEquals(['en-us', 'nl', 'en'], $client->languages());
    }
    ...
    public function testOperatingSystems()
    {
        ...
        foreach ($this->operatingSystems as $ua => $platform) {
            ...
            $this->assertEquals($platform, $client->platform(), $ua);
            $this->assertTrue($client->is($platform), $platform);

            if (!strpos($platform, ' ')) {
                $method = "is{$platform}";
                $this->assertTrue($client->{$method}(), $ua);
            }
        }
    }
    ...
    public function testBrowsers()
    {
        ...
        foreach ($this->browsers as $ua => $browser) {
            $client->setUserAgent($ua);
            $this->assertEquals($browser, $client->browser(), $ua);
            $this->assertTrue($client->is($browser), $browser);
    ...
    public function testRobots()
    {
        ...
        foreach ($this->robots as $ua => $robot) {
            $client->setUserAgent($ua);
            $this->assertTrue($client->isRobot(), $ua);
            $this->assertEquals($robot, $client->robot());
        }
    }
    ...
    public function testMobileDevices()
    {
        ...
        foreach ($this->mobileDevices as $ua => $device) {
            $client->setUserAgent($ua);
            $this->assertEquals($device, $client->device(), $ua);
            $this->assertTrue($client->isMobile(), $ua);
            $this->assertFalse($client->isDesktop(), $ua);

            if (!strpos($device, ' ')) {
                $method = "is{$device}";
                $this->assertTrue($client->{$method}(), $ua, $method);
            }
        }
    }
    ...
    public function testDesktopDevices()
    {
        ...
        foreach ($this->desktopDevices as $ua => $device) {
            $client->setUserAgent($ua);
            $this->assertEquals($device, $client->device(), $ua);
            $this->assertFalse($client->isMobile(), $ua);
            $this->assertTrue($client->isDesktop(), $ua);
    ...
    public function testVersions()
    {
        ...
        foreach ($this->browserVersions as $ua => $version) {
            ...
            $this->assertEquals($version, $client->version($browser), $ua);
        }

        foreach ($this->operatingSystemVersions as $ua => $version) {
            $client->setUserAgent($ua);
            $platform = $client->platform();
            $this->assertEquals($version, $client->version($platform), $ua);
        }

        foreach ($this->browsers as $ua => $browser) {
            $client->setUserAgent('FAKE');
            $this->assertFalse($client->version($browser));
        }
    }
    ...
    public function testIsMethods()
    {
        ...
        foreach ($this->desktops as $ua) {
            ...
            $this->assertTrue($client->isDesktop(), $ua);
            $this->assertFalse($client->isMobile(), $ua);
            $this->assertFalse($client->isTablet(), $ua);
            $this->assertFalse($client->isPhone(), $ua);
            $this->assertFalse($client->isRobot(), $ua);
        }

        foreach ($this->phones as $ua) {
            $client->setUserAgent($ua);
            $this->assertTrue($client->isPhone(), $ua);
            $this->assertTrue($client->isMobile(), $ua);
            $this->assertFalse($client->isDesktop(), $ua);
            $this->assertFalse($client->isTablet(), $ua);
            $this->assertFalse($client->isRobot(), $ua);
